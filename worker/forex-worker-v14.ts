// =============================================================================
// FOREX CALENDAR PROXY - Deno Deploy (v14 - better actuals recall + merge cache)
// =============================================================================
// WHAT CHANGED FROM v13:
//   Mid-week actuals (ADP, ISM Services, Unemployment Claims) were being missed
//   and sometimes vanished after appearing. Two root causes:
//     (a) a single Haiku call with only 5 web searches was asked to cover the
//         WHOLE week at once - including events not yet released - so it spent
//         its budget on the headline prints (NFP/CPI/ISM Mfg) and never reached
//         the mid-tier releases; and
//     (b) every run OVERWROTE the shared cache, so a weak run could wipe values
//         an earlier run had already found.
//   v14:
//     - Only asks about events whose scheduled time has already PASSED
//       (whenMs <= now) - focuses the limited web searches on data that exists.
//     - Scales the web-search budget to the number of events (min 6, cap 14) and
//       tells the model to be exhaustive, not just grab the headline prints.
//     - MERGES each run into the week's existing actuals (keyed by
//       date|currency|title, newer value wins) instead of overwriting, and
//       week-scopes the cache so it resets cleanly on a new week. Values now only
//       accumulate across the day's runs - they never disappear.
//     - Logs web_search_requests to the AI meter (Anthropic bills web search
//       separately from tokens, ~$0.01 each).
//   To push recall even higher, set ECA_MODEL to "claude-sonnet-5" (one line).
//
...
// deno-lint-ignore no-explicit-any
async function fetchActualsViaClaude(ffEvents: FFEvent[], debug: any): Promise<any[] | null> {
  if (!ANTHROPIC_API_KEY) { debug.claude = { error: "ANTHROPIC_API_KEY not set" }; return null; }
  const { from, to } = weekRange();
  // Only ask about events whose scheduled time has already PASSED. Asking about
  // not-yet-released events wastes the model's limited web searches and dilutes
  // its focus - the exact reason mid-week releases were being missed. The shared
  // cache accumulates across the day's runs (see refreshActuals merge), so
  // nothing is lost by narrowing each individual call to what actually exists.
  const nowMs = Date.now();
  const wanted = ffEvents.filter((e) => /high|medium/i.test(e.impact) && e.whenMs <= nowMs);
  const list = wanted.map((e) => `${e.currency} | ${e.title} | ${e.dateKey}`).join("\n");
  debug.claude = { asked: wanted.length, model: ECA_MODEL };
  if (!wanted.length) { debug.claude.note = "no released events yet"; return []; }
  // Give the model enough search budget to reach every released event - one search
  ...
  const maxSearches = Math.min(14, Math.max(6, Math.ceil(wanted.length / 2)));
  ...
  try {
    ...
    try { data = JSON.parse(txt); } catch { debug.claude.status = resp.status; debug.claude.raw = txt.slice(0, 300); return null; }
    debug.claude.status = resp.status;
    if (!resp.ok) { debug.claude.error = data?.error?.message || `HTTP ${resp.status}`; return null; }
  } catch (e) { debug.claude.error = msg(e); return null; }

  try { await recordEcaUsage(data?.usage); } catch { /* */ }

  const text = (data?.content ?? []).filter((b: any) => b?.type === "text").map((b: any) => b.text || "").join("\n").trim();
  debug.claude.usage = data?.usage || null;
  debug.claude.textSample = text.slice(0, 200);
  const arr = extractJsonArray(text);
  if (!arr) { debug.claude.parse = "no-json-array"; return null; }
  debug.claude.parsed = arr.length;
  return arr;
}
// deno-lint-ignore no-explicit-any
...
async function refreshActuals(): Promise<boolean> {
  ...
  actualsInFlight = (async () => {
    ...
    try {
      ...
      const claude = await fetchActualsViaClaude(ffEvents, debug);
      if (!claude) { debug.result = "no-claude"; return false; }
      const entries = matchActuals(ffEvents, claude, debug);

      // MERGE into this week's existing actuals so a single weak run can't wipe
      // values an earlier run already found (the model's coverage varies run to
      // run). Keyed by date|currency|title; the newer value wins (handles
      // corrections). Week-scoped so it resets cleanly when the week rolls over.
      ...
      for (const e of entries)      byKey.set(`${e.dateKey}|${e.country}|${e.title}`, e);
      ...
      debug.result = `matched ${entries.length}`;
...
async function serveStatus(): Promise<Response> {
  ...
  try { const d = dbg?.value || lastDebug; lastAttempt = d ? { at: d.at ? new Date(d.at).toISOString() : null, result: d.result, stored: d.stored, mergedTotal: d.mergedTotal, claude: d.claude, ffCount: d.ffCount, matched: d.matched, unmatchedClaude: d.unmatchedClaude } : null; } catch { /* */ }
  const ws = windowState();
  const status = {
