...
- **Calendar** — a month calendar of daily results. Each calendar row ends in a **week summary cell** ("Week 1", "Week 2"… with that week's money result); clicking it opens that week's **Weekly Review**. The Weekly Review shows that week's **Leaderboard achievements** in its own section after Consistency (rank, points, and the same breakdown as the leaderboard's "How X scored" popup: bias and reviews posted, weekly review, series posted, traded with your bias, the three risk rules, net R). Each risk rule (max trades per session, max daily drawdown, max risk per trade) is scored per trading day, +5 when kept and −20 when broken, and its line says how often, e.g. "kept 7 of 9 days · broken 2". The **Monthly Review** shows the month's Leaderboard achievements at the top of Monthly Insights (it replaced "Patterns worth noting").
