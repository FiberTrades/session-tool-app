...
- **Pips vs points (distance unit)** — forex trades are measured in **pips**; NQ, US100 and other indices, futures, stocks and crypto are measured in **points** (1.00 of price: 1 NQ point is $20 a contract, $2 on MNQ), or in ticks if the trader sets that in the EA. Each trade keeps its own unit, taken from the EA (Minimalist Manager EA v8.0 has a **Distance unit** input: Auto / Pips / Points / Ticks — Auto uses pips on forex and gold, points on everything else) or, for manual trades, from the trade's symbol. Every label follows it: an NQ trade's Trade Log boxes read PTS, the edit window says points, Trade Replay measures in points, and stats show points when the view contains only NQ-type trades. A view that mixes forex and index trades labels its pip totals "pips/pts" — those totals add pips and points together and are not meaningful; filter by symbol or account to see one unit. Pips and points are never converted into each other. R is the same everywhere.
