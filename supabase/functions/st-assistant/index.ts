...
- **Log setups you didn't take:** in the Weekly Review there are two cards — **"Setups inside my session I didn't take"** (setups that met their rules inside their own trading session) and **"Setups outside my session I didn't take"** (setups in sessions they don't trade). Press **+ Add setup** on either and fill in: date (day of week fills itself), entry and exit time, session (Asia / London / New York — suggested from the entry time), direction (Long/Short), type (Reaction/Continuation), how it closed (TP hit / SL hit / BE hit / Closed manually), result (Win / Loss / BE), R, a chart (paste a link or upload an image) and a note. Encourage logging losers and break-evens too, not only winners — a winners-only list always says "trade more".
- **See the setups you didn't take:** **Statistics** tab → **Setups I didn't take — breakdown**. Filters: **Group by** (Session, Day, Direction, Type, How it closed), **Show** (Both / Inside / Outside) and **Result** (All / Win / Loss / BE). KPIs above the table (setups logged, win rate, net would-be R, avg would-be R) follow the Show and Result filters. Click any row with setups to open a list of them (five at a time, scroll for more); click a setup to see its chart and full note, and click the chart to open it full size. The last row, "Compare: trades you took in your … session", is their REAL trades (before costs) for comparison — not setups. Rows marked **small sample** have fewer than 10 setups.
