...
- **Playbooks + Trade Details (Session Review):** each trade's review has **06 Trade Details**, whose first row is the **Playbook** picker (pick a playbook and its answers fill in, or answer the rows and save them as a new playbook). Trade Details replaced the old Concept Tags chips and the separate RICE section: one row per question, one tap per row - first the session context: Session (London / Asia / New York, with From-To time pickers on the same row - picking a session fills empty pickers with that session's times from Settings, e.g. London 08:00-10:00, still editable), In-session news (tick all: High / Medium / Bank Holiday); then Trade type (Reaction / Continuation; the same field as the Trade Log's Type column), Market phase (Initiation / Trending / Ranging), RICE phase (tick all), Location (Premium edge / Premium middle / 50% / Discount middle / Discount edge), Liquidity (tick all: High sweep, Low sweep, Prev D/W/M, Asia H/L, London H/L), Rejection (Strong / Slow), How did we break (Aggressive / Slow), Fractal MS creation (Yes / No), Entered at (High / 50% / Low), HTF direction (With / Against), Stop (Safe SL / Unsafe SL), Probability (High prob / Low prob), then **Mindset** (tick all: every mindset tag, e.g. Calm, Focused, FOMO, Revenge Traded, plus the member's own via **+ Custom** on that row) - the separate Mindset Tags section is gone, so Session Review is 05 Charts, 06 Trade Details, 07 Reflection. Mindset is NOT part of a playbook's recipe (never matched or shared) but it is in the Statistics breakdown. **Edit order** (beside + Add your own row) puts up/down arrows on every row to reorder them - the member's own order, used everywhere the rows appear; **Done** hides the arrows. **+ Add your own row** at the bottom (a name, answers separated by commas, and whether more than one can apply; the x on the row removes it, past trades keep their answers). Members who used Tennis Serve have it as their own row. Their own free tags stay under **More tags**. A **playbook** = a setup + its recipe (the answers it expects) + an optional description; the old Setup Tags became the Playbook picker. Answer a trade's rows and tap **Save as playbook**; next time pick it and the rows fill in. Any row that differs is flagged (pink row label, the expected answer outlined in dashed pink) and the status reads e.g. **Recipe 9/10 - Slow rejection**. **Update <name>** sets the recipe to this trade's answers (nothing changes a recipe by itself); **Save as variant** makes a new playbook from this trade, grouped under its parent. **Manage playbooks** (beside the picker): rename (renames it on every trade), description, **Edit recipe**, **Share to #playbooks**, Delete (trades keep the name as history). Playbooks are optional - a review counts without one. Old setup tags stay on their trades as history; members' existing setup tags became name-only playbooks automatically. **Community:** the Session Review post prints Playbook (with the recipe match), Session (e.g. London - 08:00-10:00 - News: High), Trade type, Location, Price action, Risk and any own rows. The **#playbooks** channel (Progress & Learning) holds shared playbook cards - only the playbook (name, description, recipe and any own rows it uses) is shared, never trades or results - and **Add to my playbooks** on a card copies it into yours to edit. To share, either Manage playbooks -> **Share to #playbooks**, or open the #playbooks channel and tap **Share a playbook** (above the message box): it lists your playbooks and posts the one you pick as the card - every card is built by the app, so they all look the same and each reader sees the labels in their own language; anything typed normally in the channel is just chat. **Saved trades:** the trade window (opened from the Calendar day view, All Trades History or Series of 10) has the same Playbook picker and Trade Details rows at the bottom, applied with Save Changes (nothing is written if you don't touch them). **Setups I didn't take** (Weekly Review cards, inside and outside your session) each have a **Playbook** picker; in Statistics -> Setups I didn't take - breakdown, **Group by** includes **Playbook**, and an opened setup shows its playbook with that playbook's recipe beside the notes. **Statistics:** **Playbook Performance** (replaced Setups Performance: one row per playbook, variants under their parent whose row is the family total, old setup tags last as history; tap a row to open it below) and **Trade Details Breakdown** (what each answer earned, across all trades or inside one playbook, plus 'Matched the recipe' vs 'Differed from it').
