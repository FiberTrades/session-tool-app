...
- **Weekly Review:** **Calendar** tab → click the week's summary cell at the end of that week's row. It holds a **Playbook recipes** summary when any of the week's trades used a playbook with a recipe ("Followed the recipe on 2 of 4 playbook trades"; from 3 such trades it also shows what the followed ones and the ones that broke it made in R; "Most often off:" names the rows most often answered differently from the recipe, and "Where it differed" lists each of those trades with what differed; the posted weekly review carries the same summary - it is not a live nudge, since the playbook and its answers are only filled in after the trade), the best trade of the week (when that trade was tagged with a playbook, its card shows "PLAYBOOK Range · recipe 4/4 ✓" under the result line, and the posted weekly review prints the same line; old setup tags are not shown there), the two "setups I didn't take" cards (below), focus for next week, what they learned and what they did well, plus a **Post to community** button (the post includes BOTH lists - setups inside my session and setups outside my session I didn't take - each setup with its playbook, and the outside ones with their session) that posts the review to the community's weekend-review channel. The three questions (focus for next week, what they learned, what they did well) are the minimum: the review cannot be posted until all three are answered, and on the leaderboard a weekly review only counts with all three (reviews posted before 25 Sep 2026 count as they were).
...
- **Dropdowns (26 Sep 2026):** on a computer every dropdown in the app (Pick a playbook, Settings, the Trade Log cells, filters...) opens the app's own list - dark, grey under the pointer, the chosen option in pink - instead of the system's (Windows' was blue). Keyboard works as usual (Space / Enter to open, arrows, Enter to pick, Escape to close). Phones keep their own native pickers.
- **Playbooks + Trade Details (Session Review):** each trade's review has **06 Playbook**: its first row is **Pick a playbook** (pick one and its answers fill in, or answer the rows and save them as a new playbook), then the question rows (called Trade Details elsewhere). They replaced the old Concept Tags chips and the separate RICE section. In Session Review the questions live in a full-width **This trade** card (26 Sep 2026) with "n / 15" at the top: each line shows the question's name and ALL its options, and the ones you pick turn white. A completion column of small circles sits under the "n / 15" count on the right - one circle per question, level with its name, turning pink when that question is answered (hidden on phones and in Edit a row mode). Thin lines separate the questions (running the full width of the card), and the answers start a little way after the names. With a playbook picked, a question whose answer differs from the recipe has its name in pink with a dot, and the recipe's answer is dashed. Save as playbook (and, with a playbook picked, Update <name> / Save as variant) sits under the This trade card at the bottom right, on the same line as + Add your own row (on phones Edit a row moves to the line below them) - answer top to bottom, then save at the end; the recipe score stays beside the picker. Saving a playbook is optional: answers save to the trade as you tap them, and a review counts without one. Edit a row puts up / down / Edit on each line and opens the row editor inside the card. With a playbook picked, a question whose answer differs from the recipe has its name in pink with a dot, and the recipe's answer is dashed. (The trade window and a playbook's recipe editor still show the answers as buttons.) One row per question, one tap per row - first the session context: Session (London / Asia / New York - there is no session-time picker; it was removed on 26 Sep 2026), In-session news (tick all: High / Medium / Bank Holiday / None - None stands alone: ticking it clears the others, ticking another clears it); then Market phase (Initiation / Trending / Ranging), RICE phase (tick all), Location (Premium edge / Premium middle / 50% / Discount middle / Discount edge), Liquidity (tick all: High sweep, Low sweep, Prev D/W/M, Asia H/L, London H/L, New York H/L), How did we break? (Aggressive / Weak), POI mitigation (Yes / No; posts in the Location line as "POI mitigated" / "No POI mitigation"), Rejection (Strong / Slow), Fractal MS creation (Yes / No), Trade type (Reaction / Continuation; the same field as the Trade Log's Type column), Entered at (High / 50% / Low), Stop (Safe SL / Unsafe SL), Probability (High prob / Low prob), HTF direction (With / Against). **Mindset is NOT part of the playbook**: it has its own section again, **07 Mindset** (tick all: every mindset tag, e.g. Calm, Focused, FOMO, Revenge Traded, plus the member's own), because how you feel on a day has nothing to do with a setup - so Session Review is 05 Charts, 06 Playbook, 07 Mindset, 08 Reflection. The Mindset tags use the section's full width. At the end of them, **+ Custom** switches on custom mode: a box to type a new tag (Enter adds it to your list and ticks it; the box stays ready for the next) and an x on every tag - one you created is deleted, a built-in one is removed and listed under Removed with + to bring it back; the trade being reviewed loses that tick, past trades keep theirs. **Done** (or Escape) ends custom mode. There is no separate Edit button. Mindset is never part of a recipe (never matched or shared); The **Manage playbooks** button sits beside the Playbook picker. In the trade window (Calendar day view, All Trades History, Series of 10) Mindset is its own collapsible block beside Trade Details, open when the trade has mindset tags. **Edit a row** (beside + Add your own row) puts ↑ ↓ and an Edit button at the top right of every row - ↑ / ↓ move the row up or down (the order is saved; a new built-in row slots in after the row it follows) - and Edit lets you: rename the row, add answers to it, remove any answer (x on each - one you added is deleted, a built-in one is hidden and listed under Removed with + to bring it back, the way the old Mindset tag editing worked), or delete the row (a deleted built-in row can be restored from Edit a row; past trades keep their answers either way; Trade type and RICE answers are fixed). **Done** leaves edit mode. **+ Add your own row** at the bottom (a name, answers separated by commas, and whether more than one can apply; the x on the row removes it, past trades keep their answers). Members who used Tennis Serve have it as their own row. The old free-tag area (**More tags**) was removed on 26 Sep 2026: to add your own labels, make a row with **+ Add your own row**. Tags already ticked on past trades are kept in the journal but no longer shown or posted. A **playbook** = a setup + its recipe (the answers it expects) + an optional description; the old Setup Tags became the Playbook picker. Answer a trade's rows and tap **Save as playbook**; next time pick it and the rows fill in. Any row that differs is flagged (pink row label, the expected answer outlined in dashed pink) and the status reads e.g. **Recipe 9/10 - Slow rejection**. **Update <name>** sets the recipe to this trade's answers (nothing changes a recipe by itself); **Save as variant** makes a new playbook from this trade, grouped under its parent. **Where to see and manage all your playbooks:** **Statistics** tab -> **Your Playbooks** (its own section, above Playbook Performance), or **Manage playbooks** beside the picker in Session Review - the same list in both: one line per playbook (name and trade count, variants indented under their parent); tap a line to open that playbook's card, and the chevron at the card's top right closes it again. Each playbook card has: its name (rename renames it on every trade), **Notes** (what the setup is, when you take it - saves as you type), its recipe with **Edit recipe**, **Examples** (**+ Add example**: paste a TradingView link or upload a screenshot - uploading is Premium, like chart uploads elsewhere, so other members only see the link field - with an optional note; shown as chart thumbnails that open the chart), **Trades (n)** (the trades tagged with it, newest first, each with **Replay** when it has candles), **Share to #playbooks** and Delete (trades keep the name as history). **+ New playbook** at the top starts one from scratch (a name, then its recipe opens). Playbooks are optional - a review counts without one. Old setup tags stay on their trades as history; members' existing setup tags became name-only playbooks automatically. **Community:** the Session Review post prints Playbook (with the recipe match), Session (e.g. London - 08:00-10:00 - News: High), Trade type, Location, Price action, Risk and any own rows. The **#playbooks** channel (Progress & Learning) holds shared playbook cards - only the playbook (name, notes, recipe, example charts and any own rows it uses) is shared, never trades or results; the card shows the example chart thumbnails - and **Add to my playbooks** on a card copies it into yours to edit. To share, either Manage playbooks -> **Share to #playbooks**, or open the #playbooks channel and tap **Share a playbook** (the button inside the message box): it lists your playbooks in a pop-up above the box and posts the one you pick as the card - every card is built by the app, so they all look the same and each reader sees the labels in their own language. Sharing is the ONLY way to post in #playbooks: the message box there is read-only for everyone (admins too) and holds only the Share a playbook button and the line 'Tap "Add to my playbooks" on any card to copy one into your own.' - no typing, files, GIFs or emoji messages. On a card you can react with an emoji, forward it, and delete your own card (admins can delete any); cards can't be edited, replied to or copied as text. **Saved trades:** the trade window (opened from the Calendar day view, All Trades History or Series of 10) has the same Playbook picker and Trade Details rows at the bottom, plus a separate Mindset block, applied with Save Changes (nothing is written if you don't touch them). **Setups I didn't take** (Weekly Review cards, inside and outside your session) each have a **Playbook** picker; in Statistics -> Setups I didn't take - breakdown, **Group by** includes **Playbook** (setups without one are grouped under "No playbook"), and an opened setup shows its playbook with that playbook's recipe beside the notes (only when a playbook was picked on that setup's card in the Weekly Review - older setups have none until one is picked there). **Statistics:** **Playbook Performance** (replaced Setups Performance: one row per playbook, variants under their parent whose row is the family total). It started fresh on 26 Sep 2026 for everyone: only trades dated from 26 Sep 2026 count, and old setup tags are not shown - nothing was deleted from anyone's journal, older trades just aren't counted there. The Trade Details Breakdown section and the Concept & Mindset Tags section (top 10 concept / mindset tags) were removed from Statistics on 26 Sep 2026 - there is no per-answer or per-tag breakdown in Statistics any more.
